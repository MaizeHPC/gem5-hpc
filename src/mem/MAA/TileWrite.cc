        ...
        for(int i = write_current; i < bound_max; i += words_per_block){
            ...
            if(CAM.find(p_block_addr) != CAM.end()){
                ...
                if((twrm.count == words_per_block || (last_elemet_set && i == last_i && twrm.count == last_word_count)) && twrm.ReadExRecv) { //  
                    ...
                    expected_response++;
                    DPRINTF(MAATileWrite, "TW[%d] %s %s TileID:%d i:%d : Sending write back dirty packet is %s\n", my_indirect_id, __func__, func_unit_names[static_cast<int>(funcUnit)], TileID, i,  writeTile_pkt->print());
